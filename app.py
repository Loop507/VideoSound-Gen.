...
class AudioGenerator:
                ...
                if start_glitch_sample < end_glitch_sample:
                    ...
                    if len(segment) > 0:
                        ...
                        if glitch_type == "repeat":
                            ...
                            glitched_segment = np.tile(segment, repeat_count)[:len(segment)]
    ...
    def apply_delay_effect(self, audio_array: np.ndarray, delay_time_data: list, feedback_data: list) -> np.ndarray:
        ...
        delay_buffer = np.zeros(self.sample_rate) # Buffer di 1 secondo (max delay 0.5s)
        write_idx = 0
        ...
        for i in range(len(delayed_audio)):
            ...
            delayed_audio[i] += delay_buffer[read_idx] * current_feedback_gain
            delay_buffer[write_idx] = current_sample + delay_buffer[read_idx] * current_feedback_gain # Add feedback to buffer
    ...
    def apply_reverb_effect(self, audio_array: np.ndarray, decay_time_data: list, mix_data: list) -> np.ndarray:
        ...
        num_delay_lines = 4 
        delay_line_buffers = [np.zeros(self.sample_rate) for _ in range(num_delay_lines)]
        ...
        for i in range(len(reverbed_audio)):
            ...
            # Più preciso usando il logaritmo per la relazione tra guadagno e tempo di decadimento
            ...
            for k in range(num_delay_lines):
                read_idx = (write_indices[k] - fixed_delay_samples[k] + self.sample_rate) % self.sample_rate
                
                wet_signal += delay_line_buffers[k][read_idx]
                
                delay_line_buffers[k][write_indices[k]] = (reverbed_audio[i] + delay_line_buffers[k][read_idx] * feedback_gain_reverb) * 0.5
                
                write_indices[k] = (write_indices[k] + 1) % self.sample_rate
            
            reverbed_audio[i] = reverbed_audio[i] * (1 - current_mix_level) + wet_signal * current_mix_level
    ...
    def apply_dynamic_filter(self, audio_array: np.ndarray, cutoff_freq_data: list, resonance_data: list) -> np.ndarray:
        ...
        for i in range(0, self.total_samples, block_size):
            ...
            if cutoff_freq > 0:
                ...
                # Per filtri in tempo reale, usare sos coefficients per stabilità
                # sos = butter(2, normal_cutoff, btype='low', analog=False, output='sos')
                # filtered_left, zi_left = sosfilt(sos, segment_left, zi=zi_left)
                # filtered_right, zi_right = sosfilt(sos, segment_right, zi=zi_right)
                
                # Per semplicità e stabilità in questo contesto, usiamo lfilter
    ...
    def apply_modulation_effect(self, audio_array: np.ndarray, modulation_depth_data: list, modulation_rate_data: list) -> np.ndarray:
        ...
        if modulated_audio.ndim == 2:
            ...
            num_channels = 1
            modulated_audio = modulated_audio.reshape(-1, 1)
        ...
        lfo_phase_left = 0.0
        lfo_phase_right = 0.0 if num_channels == 2 else 0.0
        ...
        for i in range(self.total_samples):
            ...
            lfo_value_left = np.sin(lfo_phase_left)
            modulated_audio[i, 0] *= (1 + current_depth * lfo_value_left)
            lfo_phase_left += 2 * np.pi * current_rate / self.sample_rate
            lfo_phase_left %= (2 * np.pi)

            if num_channels == 2:
                lfo_value_right = np.sin(lfo_phase_right)
                modulated_audio[i, 1] *= (1 + current_depth * lfo_value_right)
                lfo_phase_right += 2 * np.pi * current_rate / self.sample_rate
                lfo_phase_right %= (2 * np.pi)
            ...
            if luminosity_data: # Solo se l'analisi ha prodotto dati validi
                ...
                if audio_layers:
                    ...
                    for layer in audio_layers:
                        ...
                        if len(layer) != target_total_samples:
                            # Questo non dovrebbe succedere con la nuova logica, ma è una sicurezza
                            if len(layer) < target_total_samples:
                ...
                if len(final_audio) < target_total_samples:
                    if final_audio.ndim == 1:
                        ...
                        final_audio = np.pad(final_audio, ((0, target_total_samples - len(final_audio)), (0,0)))
